...
if (isset($_POST['updateprofile'])) {
  ...
  $sqlCheck = "SELECT FullName, ContactNo, dob, Address, City, Country FROM tblusers WHERE EmailId = :email";
  $queryCheck = $dbh->prepare($sqlCheck);
  $queryCheck->bindParam(':email', $email, PDO::PARAM_STR);
  $queryCheck->execute();
  $oldData = $queryCheck->fetch(PDO::FETCH_ASSOC);

  // Cek apakah ada perubahan
  if (
    $name !== $oldData['FullName'] ||
    $mobileno !== $oldData['ContactNo'] ||
    $dob !== $oldData['dob'] ||
    $adress !== $oldData['Address'] ||
    $city !== $oldData['City'] ||
              ...
              <div class="dealer_info">
                ...
                <p><?php echo htmlentities($result->Address); ?><br>
                  ...
                  <div class="profile_wrap">
                    <h5 class="uppercase underline text-center">General Settings</h5>
                    <?php
                    if ($msg) { ?>
                      <div class="succWrap"><strong>Peringatan</strong>:<?php echo htmlentities($msg); ?> </div><?php } ?>
                    <form method="post">
...
<div class="form-group" style="text-align: right;">
  <button type="submit" name="updateprofile" class="btn">
    Simpan Perubahan <span class="angle_arrow"><i class="fa fa-angle-right" aria-hidden="true"></i></span>
  </button>
</div>
                </form>
              </div>
